...
pub(crate) fn widths(window: &mut gpui::Window, family: String, size: f64, weight: f64, texts: Vec<String>) -> Vec<f64> {
    widths_with_system(window.text_system(), family, size, weight, texts)
}

pub(crate) fn widths_with_system(system: &gpui::WindowTextSystem, family: String, size: f64, weight: f64, texts: Vec<String>) -> Vec<f64> {
    ...
    texts.into_iter().map(|text| {
        text.split('\n').map(|line| {
            ...
            let shaped = system.shape_line(line.to_string().into(), gpui::px(size as f32), &[run], None);
