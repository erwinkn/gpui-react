...
This package provides the native host for `@gpui-react/core` composed with the
standard controls of `@gpui-react/kit`. The tested target is macOS arm64 with
Bun. The window starts inactive. The runtime contains no Cherry, Pierre, GPU
effect, or test-only components.

Install matching versions of `@gpui-react/core`, `@gpui-react/kit`, and
...
CommonJS `require('@gpui-react/runtime')` returns those bindings directly.
...
See the [composition guide](../../crates/gpui-react-runtime/README.md).
