...
try {
  ...
  const gpu = await run("bun", [join(fixture, "host.ts")], undefined, "gpu")
  assert.match(gpu, /React GPU component:/)
  console.log("PASS source GPU worker", gpu.trim())
  ...
  await run("bun", ["build", "--compile", join(fixture, "host.ts"), join(fixture, "worker.tsx"), join(fixture, "list-worker.tsx"), join(fixture, "interaction-worker.tsx"), join(fixture, "document-worker.tsx"), join(fixture, "gpu-worker.tsx"), "--outfile", binary], temporary)
  ...
  const packagedGpu = await run(moved, [], tmpdir(), "gpu")
  assert.match(packagedGpu, /React GPU component:/)
  console.log("PASS relocated GPU worker", packagedGpu.trim())
