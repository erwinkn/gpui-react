...
for (let attempt = 0; attempt < (process.env.BRIDGE_COUNTER_MODE === "repeat" ? 3 : 1); attempt++) {
  await runApplication(bindings, new URL(process.env.BRIDGE_COUNTER_MODE === "missing" ? "./missing-worker.tsx" : "./worker.tsx", import.meta.url), { title: "React GPUI bridge check", width: 320, height: 180 })
}
